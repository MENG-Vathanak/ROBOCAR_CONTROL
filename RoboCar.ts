...
namespace RoboCar {
    ...
    const PCA9685_ADD_1 = 0x40  // Address for PCA1 - Motor
    const PCA9685_ADD_2 = 0x41  // Address for PCA2 - Servo
    ...
    // Initialize PCA9685
    function initPCA9685(addr: number, freq: number): void {
        i2cWrite(addr, MODE1, 0x00)
        setFreq(addr, freq)
    }
    ...
    function setFreq(addr: number, freq: number): void {
        ...
        i2cWrite(addr, MODE1, newmode)       // go to sleep
        i2cWrite(addr, PRESCALE, prescale)   // set the prescaler
    ...
    // Wrapper for PCA1 (Motor)
    function setPwm1(channel: number, on: number, off: number): void {
        if (!initializedPCA1) {
            initPCA9685(PCA9685_ADD_1, 1000)
            initializedPCA1 = true
        }
        setPWM(PCA9685_ADD_1, channel, on, off)
    }

    // Wrapper for PCA2 (Servo)
    function setPwm2(channel: number, on: number, off: number): void {
        if (!initializedPCA2) {
            initPCA9685(PCA9685_ADD_2, 50)
            initializedPCA2 = true
        }
        setPWM(PCA9685_ADD_2, channel, on, off)
    }
...
//***************************************************************************************************************************************************************************************************************** */
    ...
    function forward(RoboCarSpeed: number) {
        setPwm1(enMotors.M1, 0, RoboCarSpeed)
        pins.digitalWritePin(DigitalPin.P8, 0)
        pins.digitalWritePin(DigitalPin.P9, 1)

        setPwm1(enMotors.M2, 0, RoboCarSpeed)
        pins.digitalWritePin(DigitalPin.P15, 1)
        pins.digitalWritePin(DigitalPin.P16, 0)

        setPwm1(enMotors.M3, 0, RoboCarSpeed)
        pins.digitalWritePin(DigitalPin.P10, 0)
        pins.digitalWritePin(DigitalPin.P12, 1)

        setPwm1(enMotors.M4, 0, RoboCarSpeed)
    ...
    function forwardRight(RoboCarSpeed: number) {
        ...
        setPwm1(enMotors.M2, 0, 0)
        pins.digitalWritePin(DigitalPin.P15, 0)
        pins.digitalWritePin(DigitalPin.P16, 0)

        setPwm1(enMotors.M3, 0, 0)
        pins.digitalWritePin(DigitalPin.P10, 0)
        pins.digitalWritePin(DigitalPin.P12, 0)
    ...
    export function RGB_Program(): neopixel.Strip {
        if (!yahStrip) {
            yahStrip = neopixel.create(DigitalPin.P0, 4, NeoPixelMode.RGB);                             //DigitalPin_p0 connected to RGB         
        }
        return yahStrip;
    }
    ...
    export function MotorRun(index: enMotors, speed: number): void {
        ...
        led.enable(false)
        speed = (speed * 16) * 0.55; // (map 255 to 4096)
        if (speed >= 4096 * 0.55) {
            speed = 4096 * 0.55
        }
        if (speed <= -(4096 * 0.55)) {
...
//==============================================================================
    ...
    //     i2cwrite(PCA9685_ADD_1, MODE1, 0x00)
    //     setFreq1(1000);
    //     initializedPCA1 = true
    // }
    ...
    //     let oldmode = i2cread(PCA9685_ADD_1, MODE1);
    //     let newmode = (oldmode & 0x7F) | 0x10; // sleep
    //     i2cwrite(PCA9685_ADD_1, MODE1, newmode); // go to sleep
    //     i2cwrite(PCA9685_ADD_1, PRESCALE, prescale); // set the prescaler
    //     i2cwrite(PCA9685_ADD_1, MODE1, oldmode);
    //     control.waitMicros(5000);
    //     i2cwrite(PCA9685_ADD_1, MODE1, oldmode | 0xa1);
    // }
    ...
    //     buf[3] = off & 0xff;
    //     buf[4] = (off >> 8) & 0xff;
    //     pins.i2cWriteBuffer(PCA9685_ADD_1, buf);
    // }
    // //---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    // function i2cwrite2(addr: number, reg: number, value: number) {
    //     let buf = pins.createBuffer(2)
    //     buf[0] = reg
    //     buf[1] = value
    //     pins.i2cWriteBuffer(addr, buf)
    // }
    // function i2ccmd2(addr: number, value: number) {
    //     let buf = pins.createBuffer(1)
    //     buf[0] = value
    //     pins.i2cWriteBuffer(addr, buf)
    // }
    // function i2cread2(addr: number, reg: number) {
    //     pins.i2cWriteNumber(addr, reg, NumberFormat.UInt8BE);
    //     let val = pins.i2cReadNumber(addr, NumberFormat.UInt8BE);
    //     return val;
    // }
    // function initPCA9685_2(): void {
    //     i2cwrite2(PCA9685_ADD_2, MODE1, 0x00)
    //     setFreq2(50);
    //     initializedPCA2 = true
    // }
    ...
    //     prescaleval -= 1;
    //     let prescale = prescaleval; //Math.Floor(prescaleval + 0.5);
    //     let oldmode = i2cread2(PCA9685_ADD_2, MODE1);
    //     let newmode = (oldmode & 0x7F) | 0x10; // sleep
    //     i2cwrite2(PCA9685_ADD_2, MODE1, newmode); // go to sleep
    //     i2cwrite2(PCA9685_ADD_2, PRESCALE, prescale); // set the prescaler
    //     i2cwrite2(PCA9685_ADD_2, MODE1, oldmode);
    //     control.waitMicros(5000);
    //     i2cwrite2(PCA9685_ADD_2, MODE1, oldmode | 0xa1);
    // }
    // function setPwm2(channel: number, on: number, off: number): void {
    //     if (channel < 0 || channel > 15)
    //         return;
